		...
		<div class="soon">
			...
			<div>
				...
				<div class="contents">
					...
					<div id="rooms_info">
						<c:if
							test="${not empty adminGoodsList && adminGoodsList != 'null' }">
							<c:forEach var="item" items="${adminGoodsList}"
								varStatus="status">
								<form name="" method="POST">
									<input type="hidden" name="goodsId" value="${item.goodsId}">
									<div class="rooms">
										<div class="room_img_wrap">
											<input type="hidden" name="goodsImageFileName"
												value="${item.goodsImageFileName}"> <img
												src="${contextPath}/gThumbDownload.do?goodsImageFileName=${item.goodsImageFileName}"
												alt="상품 이미지" />
										</div>
										<div class="room_info_wrap">
											<input type="hidden" name="goodsName"
												value="${item.goodsName}"> <b>상품명</b>
										...
										<div class="room_wrap">
											<div class="room_price">
												<b>회사명</b><br> <b>${item.companyName}</b><br>
												<br> <b>남은 객실 : ${item.goodsCount}</b>
											</div>
										</div>
										<div class="room_wrap">
											<div class="room_price">
												<input type="hidden" name="goodsPrice"
													value="${item.goodsPrice}"> <b>${item.goodsPrice}원</b>
											</div>
										</div>

										<div class="room_wrap">
											<div class="goods_button_wrap">
												<input type="button" class="admin_btn"
													onclick="location='/admin/adminGoodsAllow.do?goodsId=${item.goodsId}'"
													value="상품 승인하기" />
						...
						<c:choose>
							...
							<c:otherwise>
								<button id="userListpage_button1">
									<a href="${contextPath}/admin/adminGoodsList.do"
